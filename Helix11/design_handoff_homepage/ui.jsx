/* ============================================================
   HelixOS AI — UI primitives + icon set
   ============================================================ */
const { useState, useRef, useEffect, useMemo } = React;

// ---- Icons (clean 1.6px stroke, 24 grid) ------------------------------
const PATHS = {
...
// ---- Card -------------------------------------------------------------
function Card({ children, style, pad = "var(--s5)", className, onClick, hover }) {
  ...
  return (
    <div onClick={onClick}
      ...
      style={{
        ...
        borderRadius: "var(--r3)", padding: pad,
...
function Badge({ children, tone = "neutral", dot, icon, style }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: 99, fontSize: 11.5, fontWeight: 600, letterSpacing: ".01em",
      color: fg, background: bg, lineHeight: 1.4, whiteSpace: "nowrap", ...style,
    }}>
...
// ---- Button -----------------------------------------------------------
function Button({ children, variant = "default", size = "md", icon, iconRight, onClick, disabled, style, full }) {
  const [h, setH] = useState(false);
...
// ---- Agent glyph chip -------------------------------------------------
const AGENT_HUE = { founder: 48, operations: 250, marketing: 320, analytics: 155, sales: 25, research: 200, finance: 90, support: 290 };
function AgentGlyph({ id, size = 36, active, status }) {
  const a = window.HELIX.agentById(id);
...
// ---- Status dot -------------------------------------------------------
const STATUS_META = {
  idle: ["neutral", "Idle"], working: ["accent", "Working"], waiting: ["warn", "Waiting approval"],
  done: ["ok", "Done"], locked: ["neutral", "Locked"], queued: ["info", "Queued"],
  running: ["accent", "Running"], approved: ["ok", "Approved"],
};
...
function Sparkline({ data, w = 240, h = 56, stroke = "var(--accent)", fill = true, dots }) {
...
// ---- Empty state ------------------------------------------------------
function Empty({ icon = "scenario", title, text }) {
  return (
...
// ---- number format ----------------------------------------------------
const fmt = {
  k: (n) => n >= 1000 ? (n / 1000).toFixed(n >= 10000 ? 1 : 2).replace(/\.0$/, "") + "k" : "" + n,
  money: (n) => "$" + (n >= 1000 ? (n / 1000).toFixed(1).replace(/\.0$/, "") + "k" : n),
  pct: (n) => (n * 100).toFixed(1) + "%",
  int: (n) => n.toLocaleString("en-US"),
};

Object.assign(window, {
  Icon, Card, Badge, Button, AgentGlyph, StatusPill, Sparkline, SectionTitle, Empty, fmt,
  AGENT_HUE, STATUS_META,
});
