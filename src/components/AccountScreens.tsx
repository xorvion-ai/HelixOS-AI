      ...
      <Card style={{ background: "linear-gradient(120deg, var(--accent-soft), var(--surface) 70%)", borderColor: "var(--accent-line)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
          {me?.picture ? (
            ...
            <img src={me.picture} alt="" referrerPolicy="no-referrer" style={{ width: 76, height: 76, borderRadius: 18, objectFit: "cover" }} />
          ) : (
          ...
          <Button variant="default" icon="edit" onClick={() => setEditing(true)}>Edit profile</Button>
        </div>
      </Card>

      {editing && <EditProfileModal currentName={name} onClose={() => setEditing(false)} onSaved={refresh} />}
...
const TERMS: [string, string][] = [
  ["1. Acceptance of terms", "By creating an account or using HelixOS AI (operated by Xorvion), you agree to these Terms & Conditions. If you are using HelixOS on behalf of an organisation, you confirm you have authority to bind that organisation to these terms."],
  ["2. The service", "HelixOS provides an autonomous multi-agent workspace that plans, runs, and analyses business activity on your behalf. Agent outputs are generated by AI and are provided for your decision-making support — they are not professional, legal, or financial advice."],
  ["3. Your account", "You are responsible for the activity under your account and for keeping your sign-in credentials secure. One workspace is provisioned per user; you must not share access in a way that misrepresents who is using the service."],
  ["4. Acceptable use", "You agree not to use HelixOS to break the law, to send spam or abusive outreach, to infringe others' rights, or to attempt to disrupt, reverse-engineer, or overload the platform. Sensitive agent actions are gated behind human approval, and you remain accountable for the actions you approve."],
  ["5. Your content", "You retain ownership of the scenarios, documents, goals, and other content you add. You grant us a limited licence to process that content solely to operate the service (run agents, RAG retrieval, memory recall, and dashboards). We do not use your content to train foundation models."],
  ["6. AI output & accuracy", "AI-generated content can be incomplete or inaccurate. You are responsible for reviewing agent outputs before acting on them, especially for outreach, spend, and customer-facing actions. HelixOS provides observability traces so you can audit every decision."],
  ["7. Free tier & availability", "The service currently runs on free-tier infrastructure and is provided “as is”, without warranty of uninterrupted availability. We may change, suspend, or discontinue features at any time, and limits may apply to usage."],
  ["8. Termination", "You may stop using HelixOS and delete your workspace at any time. We may suspend or terminate access if these terms are breached. On termination, your workspace content and embeddings are removed as described in the Privacy Policy."],
  ["9. Liability", "To the maximum extent permitted by law, HelixOS and Xorvion are not liable for indirect or consequential losses, or for decisions made on the basis of agent output. Nothing in these terms excludes liability that cannot be excluded by law."],
  ["10. Changes & contact", "We may update these terms; material changes will be reflected by the “last updated” date below. Questions about these terms can be sent to xorvion.ai@gmail.com."],
];
...
export function TermsScreen() {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "var(--s5)", maxWidth: 880 }}>
      <Card style={{ background: "linear-gradient(120deg, var(--accent-soft), var(--surface) 75%)", borderColor: "var(--accent-line)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 46, height: 46, borderRadius: 12, background: "var(--surface)", display: "grid", placeItems: "center", color: "var(--accent-strong)", border: "1px solid var(--accent-line)" }}><Icon name="scroll" size={22} /></div>
          <div>
            <div style={{ fontSize: 17, fontWeight: 700 }}>Terms &amp; Conditions</div>
            <div className="mono" style={{ fontSize: 12, color: "var(--text-2)", marginTop: 3, letterSpacing: ".04em" }}>Last updated · June 2026</div>
          </div>
        </div>
      </Card>
      {TERMS.map(([h, body]) => (
...
// ---- Edit profile modal -----------------------------------------------
function EditProfileModal({ currentName, onClose, onSaved }: { currentName: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    if (!next) { setErr("Please enter a name."); return; }
    const sb = getBrowserSupabase();
    if (!sb) { setErr("Editing your profile needs you to be signed in."); return; }
    setBusy(true);
    try {
      const { error } = await sb.auth.updateUser({ data: { full_name: next, name: next } });
      if (error) { setErr(error.message); return; }
      onSaved();      // refresh /api/me so the new name shows everywhere
      onClose();
    } catch {
  ...
  return (
    <div onClick={onClose}
      style={{ position: "fixed", inset: 0, zIndex: 100, background: "color-mix(in oklch, var(--bg), transparent 30%)", backdropFilter: "blur(4px)", display: "grid", placeItems: "center", padding: "var(--s5)" }}>
      <div onClick={(e) => e.stopPropagation()}
        style={{ width: "min(440px, 100%)", background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r3)", boxShadow: "var(--shadow-lg)", padding: "var(--s6, 24px)", display: "flex", flexDirection: "column", gap: "var(--s4)" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ fontSize: 17, fontWeight: 700, letterSpacing: "-.01em" }}>Edit profile</div>
          <button type="button" aria-label="Close" onClick={onClose} style={{ background: "transparent", border: "none", color: "var(--text-3)", cursor: "pointer", display: "grid", placeItems: "center" }}><Icon name="x" size={18} /></button>
        </div>
        <div>
          <label style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text-2)", display: "block", marginBottom: 6 }}>Display name</label>
          <input style={inp} value={name} placeholder="Your name" autoFocus
